...
void MatamStory :: readPlayers(std::istream& playersStream){
    ...
    while (playersStream >> name >> job >> character) {
        ...
        if (character == "Responsible") {
            ...
            throw std::runtime_error("Invalid Players type");
        }
...
void MatamStory::add_Pack(std::istream& eventsStream) {
    ...
    for (int i = 0; i < numMembers; ++i) {
        ...
        if (monsterType == "Snail") {
            ...
            // Create a separate vector for the nested Pack members
            std::vector<std::unique_ptr<Monster>> subPackMembers;
            add_Pack(eventsStream, subPackMembers);  // Recursively create the nested Pack
...
void MatamStory::add_Pack(std::istream& eventsStream, std::vector<std::unique_ptr<Monster>>& members) {
    int numMembers;
    std::string monsterType;
    if (!(eventsStream >> numMembers) || numMembers < 2) {
        throw std::runtime_error("Invalid Events File");
    ...
    for (int i = 0; i < numMembers; ++i) {
        ...
        if (monsterType == "Snail") {
            ...
            // Recursively handle the nested Pack
            std::vector<std::unique_ptr<Monster>> subPackMembers;
            add_Pack(eventsStream, subPackMembers);  // Handle nested packs
            members.push_back(std::make_unique<Pack>(std::move(subPackMembers)));
        } else {
            throw std::runtime_error("Invalid Events File");
        }
